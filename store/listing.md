...
### 0.1 名称与商标（2.8.11 已更名）

当前名称为 **影哨**（原名「Bili 缓冲站」，2026-09-24 起更名）：影＝影片，哨＝在前方守望，对应「提前一格」的原创蓝青色图标，不含 Bili / B 站关键词，非官方身份最清晰。
...
名称已同步 `manifest.json`、`popup.html`、`README.md`、隐私页与图标内嵌文案。由于名称不再含「bilibili / B 站」，搜索曝光依赖 **Summary 与详细描述**：两者已保留「B 站」指名性描述（说明适用于哪个网站，不暗示授权），详情页首段的非官方免责声明必须保留。

历史决策记录（更名前评估的三种方案）：A `缓冲站 · B 站视频缓存助手`（低风险）；B 保持 `Bili 缓冲站`（中风险，需显著免责声明）；C `缓冲站 · 网页视频离线缓存`（最低风险但曝光差）。最终采用了第四条路——独创名「影哨」，规避商标问题后用描述字段补兼容性关键词。
...
| 字段 | 填写内容 |
| --- | --- |
| **Item name** | `影哨`（2 字，上限 75 字；更名背景见 0.1） |
...
```
影哨把 B 站视频提前缓存到本机，之后可以离线播放或另存为文件。它只在你点击按钮或打开开关后工作，不修改网页内容，也没有服务器。
