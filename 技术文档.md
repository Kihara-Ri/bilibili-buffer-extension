...
2. **播放**：提前加载开关与当前状态在前；概览条常驻「加速网速 / 已命中缓存 / 预热就绪」三个数字，CDN 路线与最大并发压成一行低频调节（语义写进悬停提示）；「进度条外观」与「详细统计」默认折叠——外观摘要行本身是实时配色预览，展开后提供已播放颜色、单独标出提前加载开关、预设/自定义配色和恢复默认，统计折叠区收纳播放器缓冲、当前并发、救援、回退与最近节点。关闭颜色区分不关闭提前加载，示意条不是实时进度。
...
当前急需请求使用 64 KiB 网络分块，后台预热使用 256 KiB。共享队列初始 8 路，默认全局上限 32，可配置 1–32。单个急需请求的普通工作者数为 `min(上限, max(1, ceil(有效范围字节数 / 256 KiB)))`，因此 2 MiB 用 8 路、8 MiB 用 32 路；64 KiB 网络分块保持不变，救援仍受共享全局预算限制。真实播放缓冲不足 3 秒立即放开全局预算，而不是把每个小请求都开到 32 路；不足 10 秒或接收吞吐低于估计播放消耗的 1.25 倍，每 500 ms 可翻倍增援。播放消耗按当前活跃轨道总大小 / 视频时长 × 播放倍速估算。预热驻留量不能掩盖播放器实际缺缓冲。音频优先于视频当前需求，两者优先于后台预热；后台最多占用一半连接。失败冷却对应节点，不再全局减半或永久停用轨道。预取失败按 2–30 秒退避后恢复；累计已读字节只用于统计，不作为永久停止条件。
...
配置新增 cdnMode（mainland/auto/original）、networkPolicyVersion=3、maxConcurrency 默认 32。实测后，新配置的 cdnMode 默认 original（仅使用清单主备节点）；已有明确保存的模式/上限保留，不强行覆盖。旧策略迁移到新全局上限。设置沿用串行持久化与失败回滚，统计刷新不替换交互节点。
