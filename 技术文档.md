...
主要文件：`popup.html`、`popup.css`、`src/popup.js`、`src/task-presentation.js`
...
2. **播放**：提前加载开关与当前状态在前；概览条常驻「加速网速 / 已命中缓存 / 预热就绪」三个数字；「进度条外观」与「详细统计」默认折叠——外观摘要行内嵌唯一的实时配色示意条，展开后只有图例、已播放颜色、单独标出提前加载开关、预设/自定义配色与底部「恢复默认」，统计折叠区收纳播放器缓冲、当前并发、救援、回退与最近节点。CDN 路线与并发上限由系统策略决定（原线路优先、32 路上限内自适应），不提供手动选择。关闭颜色区分不关闭提前加载，示意条不是实时进度。
...
实时刷新按业务 ID 复用片库节点，按 `createdAt` 与 ID 稳定排序；过期快照用任务世代/时间戳与删除墓碑拦截，异步操作的 pending 不被刷新重置。速度按时间平滑，数字/单位单行，进度条使用浮点比例和 transform。多任务徽标由数据库任务集合派生：单任务进度、多个任务数量、无活动任务时完成/错误状态；SW 重启从数据库恢复。配色请求串行保存，失败回到最后确认配置，而非旧乐观值。完整原理与测试见 [Popup 稳定性设计](docs/popup-stability-design.md)。
...
- 管理 Popup 快照和播放辅助配置。
- 运行浏览器侧能力自检并记录结论（Chrome 更新后强制刷新一次）。
...
- `downloadedBytes`：已接收字节，可能包含重试时丢弃的在途数据，不直接标成“已缓存”；
- `resumeBytes`：数据库中连续、完整、原子写入的安全断点；协调器在事务执行中可暂存下一断点，但不得对外当成成功；
- `committedBytes`：只有事务完成后才能提高的对外确认水位，Popup 的已缓存数字和下载阶段进度以此为准；旧记录回退到安全断点，合并/提取阶段保持独立语义；
- `createdAt`：任务稳定创建顺序；`runStartedAt`：下载世代，区分旧快照与真正重新下载；`updatedAt` 只参与新旧比较，不决定行位置。
...
当前急需请求使用 64 KiB 网络分块，后台预热使用 256 KiB。共享队列初始 8 路，全局上限固定 32 路，实际并发由收益自适应策略在限内自行决定，不再接受用户配置。单个急需请求的普通工作者数为 `min(上限, max(1, ceil(有效范围字节数 / 256 KiB)))`，因此 2 MiB 用 8 路、8 MiB 用 32 路；64 KiB 网络分块保持不变，救援仍受共享全局预算限制。真实播放缓冲不足 3 秒立即放开全局预算，而不是把每个小请求都开到 32 路；不足 10 秒或接收吞吐低于估计播放消耗的 1.25 倍，每 500 ms 可翻倍增援。播放消耗按当前活跃轨道总大小 / 视频时长 × 播放倍速估算。预热驻留量不能掩盖播放器实际缺缓冲。音频优先于视频当前需求，两者优先于后台预热；后台最多占用一半连接。失败冷却对应节点，不再全局减半或永久停用轨道。预取失败按 2–30 秒退避后恢复；累计已读字节只用于统计，不作为永久停止条件。

单次媒体请求最多 16 MiB、总截止 25 秒，单连接 10 秒；首字节 3.5 秒、正文停滞 2.5 秒可失败换节点；700 ms 未收到字节可启用备用副本。所有块校验通过后才写入缓存或交给播放器，不能把缺块伪装成成功。
...
- 不设累计字节停止线。未完整命中时尝试按需下载，失败或不支持的范围回退原请求；不返回部分假命中。
...
- 通过 `toReleaseManifest`（`scripts/lib/release-checks.mjs`）删除 `http://127.0.0.1/*` 主机权限，
  开发态清单与发布态清单共用同一份变换规则，避免构建与校验各自维护一套；
- 复制 `privacy.html`（弹窗页脚指向它，离线可用）；
- 校验 Manifest 引用文件；
- 生成根目录结构正确的 ZIP。

随后自动执行 `npm run verify`（`scripts/verify-release.mjs`）做上架体检，见第 15 节。

## 10. 权限说明

| 权限 | 用途 |
| --- | --- |
| `activeTab` | 读取当前标签页地址，判断它是否为支持的视频页；不用于读取页面内容或注入脚本 |
...
Chrome Web Store 的政策要求与代码之间存在几处“改代码就会踩到”的耦合，集中记录在这里。

### 15.1 扩展 ID 必须与 DNR 规则一致

`rules/cdn-headers.json` 通过 `condition.initiatorDomains` 把“补 B 站 Referer”限制在本扩展自己发起的请求上，
而 DNR 对扩展请求匹配的是**扩展 ID 这个主机名**。扩展 ID 由 `manifest.json` 的 `key` 推导
（公钥 DER 的 SHA-256 前 16 字节，每个半字节映射到 a–p），因此：

- 删除或更换 `key` 会改变扩展 ID，规则立即失效，CDN 开始返回 403；
- 规则里写成站点域名（如 `www.bilibili.com`）会变成修改其他页面发起的请求，属于越权。

`checkDnrRules` 会同时拒绝这两种情况；不要手工推算 ID，直接用
`node scripts/verify-release.mjs` 或测试里的 `extensionIdFromKey`。

### 15.2 权限最小化

`ALLOWED_PERMISSIONS` 是权限白名单，`npm run verify` 会对发布清单做硬校验：
出现白名单外的权限就直接失败。新增权限时必须同时更新白名单、`store/listing.md` 的理由表与本节的用途说明。

### 15.3 图标规范

当前采用「提前一格」矢量图标：蓝色播放主体与两段青色缓冲块。母版只有填充路径，无位图、字体、滤镜或外部资源；小尺寸母版加宽分段间隙，保证缩至 16 像素仍能分辨三段。
Chrome 不支持在 manifest 图标字段使用 SVG，实际加载由母版导出的 PNG。

Chrome 对方形图标建议在 128 画布内放置 96×96 的图形，四周各留 16 像素完全透明；本项目的新图标也采用至少 16 像素边距——工具栏图标会叠在深色或彩色底上，
不透明底色会变成一块突兀的方块。1.0 时代的 PNG 是拍平导出的（整张 128×128 全是不透明白底），
因此改为由 `scripts/build-icons.mjs` 从 SVG 母版渲染并自检：四角 alpha 必须为 0，48/128 的留白不少于 12.5%，
16/32 至少留 1 像素（按比例只剩 2 像素会让图形糊成一团，这两个尺寸单独使用简化母版 `icon-small.svg`）。

### 15.4 隐私与数据处理

扩展没有服务端，所有数据都在本机。为了让这一点在商店审查与用户侧都可验证：

- `privacy.html` 是唯一数据来源说明，同时打包进扩展（弹窗页脚常驻入口）并可用于 GitHub Pages；
- 页面必须自包含（无外部脚本、无外部样式），离线打开与商店抓取都能正常显示；
- Limited Use 声明必须以连续原文出现，不能被标签切断；
- 2026 年 7 月的政策更新要求“安装后也要显著披露数据处理方式”，这就是弹窗常驻页脚存在的原因。

### 15.5 校验分层

| 层 | 位置 | 覆盖 |
| --- | --- | --- |
| 语法 | `scripts/check-syntax.mjs` | 遍历仓库所有 `.js` / `.mjs` 跑 `node --check` |
| 规则 | `scripts/lib/release-checks.mjs` | 纯函数判断，有单元测试喂坏样本，防止条件写反后“永远通过” |
| 单元 | `tests/store-compliance.test.mjs` | 真实文件符合要求 + 坏样本必须被拒绝 |
| 发布包 | `scripts/verify-release.mjs` | 对 `dist/unpacked` 与 zip 做文件级检查，打印 SHA-256 |
...
| 发布态清单变换与上架规则 | `scripts/lib/release-checks.mjs` |
| 上架体检 | `scripts/verify-release.mjs`、`scripts/lib/png-alpha.mjs` |
| 语法检查 | `scripts/check-syntax.mjs` |
| 浏览器侧自检 | `src/health-check.js` |
| Chrome 版本门禁与同版验收 | `browser-compat.json`、`scripts/check-browser-compat.mjs`、`scripts/test-chrome.mjs`、`scripts/lib/browser-compat.mjs`、`scripts/lib/browser-launch.mjs` |
| 图标与商店素材生成 | `scripts/build-icons.mjs`、`scripts/store-assets.mjs`、`store/scene/` |
| 隐私政策 | `privacy.html` |
| 提审资料 | `store/listing.md`、`store/README.md` |


## 2.8.0 在线网络调度与统计契约

MAIN world 按 cache → routes → network → observer 加载。观察器捕获初始与 fetch/XHR playurl 同一 representation 的主备 URL。routes 在签名路径和查询串不变的前提下，生成经过白名单限制的大陆 CDN 主机候选；并不保证签名跨节点通用，403 节点冷却 60 秒，并保留原地址。原线路模式不生成额外候选。SPA 导航清空清单、队列、缓存和计数，不跨编码、清晰度或签名复用。

播放器首次合法 Range 请求直接接入 loadPlayerRange。相同 URL/范围共享在途任务，取消最后一个订阅者时取消下载；当前需求取消重叠后台任务。总长未知时先探测 64 KiB，再并行分块；严格检查 HTTP 206、Content-Range、总长和实际正文长度，按偏移有序重组。未支持的开放范围、超过 16 MiB 的请求或失败会交回原生播放器。XHR 保持 timeout、abort、open 重用与一次 loadstart 的语义。

共享队列与退避参数见 6.5。缓存总驻留上限 128 MiB、后台在途预算和约 45 秒前方窗口继续生效。离线片库的 IndexedDB 下载器不受此策略改变。

page→bridge→Popup 的 stats 包括 networkSpeed（按需与预热接收 B/s，时间平滑）、networkActive、networkLimit、networkHost（仅主机名）、networkRescues、networkReceivedMB、prefetchAheadSec、acceleratedRequests、acceleratedMB、nativeFallbacks、blockedNodes、cdnMode。网络字节包含竞争副本；cacheHitMB 只表示真实内存缓存命中。预热就绪是音视频 SIDX 完整驻留交集，与 video.buffered 独立显示。

配置新增 cdnMode（mainland/auto/original）、networkPolicyVersion=3、maxConcurrency 默认 32。实测后，新配置的 cdnMode 默认 original（仅使用清单主备节点）；已有明确保存的模式/上限保留，不强行覆盖。旧策略迁移到新全局上限。设置沿用串行持久化与失败回滚，统计刷新不替换交互节点。2.8.10 起两者退出用户配置：`sanitizeAssistConfig` 与桥的归一化一律钉死 original/32，历史保存值（含旧 mainland/auto 与自定义上限）随读取即失效，实际并发由收益自适应策略在 32 路上限内自行决定。

验证：npm test、npm run check、npm run test:network、npm run test:playback、npm run test:popup、npm run test:extension。受控网络测试与真实 B 站线路测试分开报告。真实 Chrome 的 1–128 路矩阵、最终生产策略与完整请求链路验收见 [并发验证报告](docs/concurrency-validation.md)；`npm run bench:real` 只连接已有浏览器，媒体签名与登录态不写出。候选来源和 MIT 许可见 THIRD_PARTY_NOTICES.md。
...
## 2.8.8 · 浏览器版本兼容门禁与运行期自检

**背景**：2026-09-23 Chrome 升到 154 后插件不可用。复核结论是三条事实：扩展代码与 154 兼容
（与用户同构建的 Chrome for Testing 154.0.8037.57 上四个浏览器套件全绿，并用真实扩展验证了
Offscreen 离线下载与预算租约）；该用户配置里扩展被停用（`disable_reasons = [1]`）且是解包安装，
停用后内容脚本、DNR 与 SW 均不运行；验收只跑在比用户低一档的 Chrome（153）上，版本漂移无人发现。
详见 [浏览器版本兼容与自检](docs/browser-compat.md)。

**验收夹具修正**：2.8.2 引入共享下载预算后，`tests/offscreen-integration.mjs` 与
`tests/offscreen-merge-integration.mjs` 的消息桩只回 `{ok:true}`，下载器等不到 `lease`，
在「等待完成」处静默超时，弹窗与离线下载链路实际没有覆盖。夹具改为直接调用
`src/request-budget.js` 的 `transitionBudget`（`tests/fixtures/fake-budget.mjs`），
并断言租约的申请与归还，后台改策略时夹具不再漂移。

**版本门禁**：`browser-compat.json` 记录已人工验收的 Chrome 主版本、构建号、日期与证据；
`npm run check` 增加 `scripts/check-browser-compat.mjs`，本机 Chrome 或验收浏览器跑到未验收
主版本时直接失败并给出下一步命令（探测不到浏览器时明确跳过）。`npm run test:chrome` 取与本机
Chrome 同版本的 Chrome for Testing（缓存于 `.tmp/cft-cache/`）重跑全部浏览器套件，套件通过
`scripts/lib/browser-launch.mjs` 的 `PLAYWRIGHT_CHROMIUM_EXECUTABLE` 覆盖启动浏览器。
`manifest.minimum_chrome_version` 保持 116：该字段是 API 下限，版本漂移属于上限问题。

**运行期自检**：`src/health-check.js` 逐项探测 `storage.local`、`storage.session`、IndexedDB
（真实开库一次）、DNR 静态规则集、`offscreen` + `runtime.getContexts`、`downloads`、`cookies`
与浏览器版本；单项 3 秒超时，任何失败都不向上抛，结论写入 `browserHealthV1`（6 小时复用，
`onInstalled` 的 `chrome_update` 强制刷新）。弹窗仅在失败或读不到结论时显示区块，列出失败项
与 `chrome://extensions` 排查入口，按 `data-health-check` 稳定 ID 复用节点。自检无法发现
「扩展被停用」，这一条由文档与弹窗固定文案负责说明。


## 2.8.7 · 兼容与回退

新增的增强能力全部是可选路径：缺失 `indexedDB`、`atob`/`btoa`、`window.postMessage`、`crypto.randomUUID` 或 `MutationObserver` 时降级为原生播放路径，且不得在注入期抛错。镜像写入器（`src/source-mirror.js`）按下载任务配额串行写入，连续 3 次失败即停写；页面读取超时 1.5 秒、迟到回包丢弃、连续 3 次失败暂停 60 秒。重复安装与重复写入幂等，桥只转发形状合法且仅含白名单字段的请求。`ASSIST_DEFAULTS` 与 manifest 权限保持 2.8.0 以来的取值。详见 docs/compat-fallback.md。


## 2.8.6 · 持久化源范围复用

离线下载校验过的原始 CDN 范围镜像到 Service Worker 的独立 IndexedDB 库（64 MiB / 256 行 / 跟随签名有效期）。内容身份是「pathname + total」，主机、查询串与清晰度不参与匹配，与 `playback-routes.js` 的主备 CDN 同文件假设一致；换账户后分区不同即整体失效。页面侧只对本次会话 playurl 授权过的精确地址发起读取，要求连续覆盖、单次 ≤ 4 MiB、每页单在途，命中后由观察器写入页面缓存并单列 `reuseHits`/`reuseMB`，不计入 CDN 吞吐与节点健康度。未命中、超时、连续失败、桥不可用或模块缺失一律回到原生网络路径；合并 MP4 与抽取音轨永不进入镜像。首次请求通常缺少总长，因此命中从第二次请求开始。详见 docs/persistent-range-reuse.md。
