...
10. progressive 要求完整单文件；DASH 要求视频轨和音频轨分别完整，才允许把聚合状态改成 `complete`。
...
页面主世界不能使用 `chrome.storage`，因此 `src/playback-bridge.js` 运行在 ISOLATED world，只负责从 `chrome.storage.local["playbackAssistConfigV1"]` 读取配置、通过 `window.postMessage` 下发，并把聚合统计保存在该标签页内存中供 Popup 查询。默认 `auto` 只对已经出现慢请求的轨道工作；`observe` 保留观测但不预热；`off` 停止主动辅助。关闭 Popup 不会停止页面观察或预热，关闭视频标签页则自然终止这条播放辅助链，不影响已交给 Offscreen 的完整缓存任务。
...
运行 `npm run dev` 后，本地服务只监听 `127.0.0.1:17321`。它通过文件系统监听 JS、JSON、HTML、CSS、PNG 与 SVG 的变化，并在 `/health` 返回递增修订号。扩展后台每 750 ms 轮询该端点，发现修订变化后调用 `chrome.runtime.reload()`。
...
- 下载恢复时优先向 B 站播放接口刷新 URL；如果原标签页已经关闭或页面会话不可用，则使用任务开始时保存的 CDN 候选地址继续。候选地址不含 Cookie，但仍可能按 B 站策略过期，届时需重新打开原视频后继续。
...
热更新是与上述业务状态并行的运行事件：修订号变化后 `chrome.runtime.reload()` 会终止当前内存请求，但不清除 IndexedDB；新 Service Worker 从最后原子提交的 `resumeBytes` 重新进入“恢复任务”。
...
单元测试当前共 35 项。在原有 URL、画质、保存、本地播放、固定 Range、CDN 回退、原子续传和 Popup 快照覆盖之外，新增整份 MAIN world IIFE 的 document_start 启动、host+pathname 隔离、预热闸门、动态估计器关联清洗、编码选项、旧缓存编码兼容，以及“成功但慢 TTFB 时后续分块主动切换 CDN”。`npm run check` 覆盖所有运行脚本与构建脚本；`npm run build` 生成的 v2.0.0 zip 根目录含完整 Manifest 引用文件，且扫描不到 `127.0.0.1`、`dev-reload` 或 `startDevReload`。
...
- 增加发布构建，自动移除开发热更新与本机权限；35 项单元测试、三组真实浏览器集成测试、语法检查和发布包完整性检查全部通过。
