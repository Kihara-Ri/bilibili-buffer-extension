...
## 2.8.12 · 页眉副标题与 GitHub 入口

- 弹窗页眉在主名「影哨」后新增副标题 `bilibili buffer`：主名不含 bilibili 关键词（2.8.11 的商标考量），副标题以指名性描述补足用途说明；基线对齐、等宽数码字体、不换行，页眉高度不变。
- 页眉右侧新增 GitHub 图标（内联 SVG，无外部资源），点击在新标签页打开项目仓库 `https://github.com/Kihara-Ri/bilibili-buffer-extension`，带 `rel="noopener noreferrer"` 与 `aria-label`。
- 扩展名称、manifest、仓库与产物命名保持 2.8.11 状态不变；`popup-layout.test.mjs` 新增页眉契约断言（副标题文案、链接地址、noopener 与 aria-label）。
- 版本 2.8.11 → 2.8.12。
- 验证：npm test、npm run check、npm run test:popup、npm run build（产物 `yingsao-2.8.12.zip`）。

## 2.8.11 · 更名为「影哨」

- **名称**：应用由「Bili 缓冲站」更名为**影哨**——影（影片/放映）＋哨（在播放位置前方守望），命名同时覆盖在线提前加载（哨在前方）与离线片库（影即内容）两条产品线，并呼应「提前一格」的蓝青色图标；不含 Bili / B 站关键词，规避商店商标政策风险（见 store/listing.md 0.1 的评估记录）。
- **替换范围**：manifest `name` 与 `default_title`、弹窗标题与页身、隐私页、商店资料（listing/README）、控制台日志前缀（`[影哨]`）、工具栏徽标 fallback 标题、图标 SVG 内嵌 `<title>/<desc>`、热更新服务启动文案、商店素材生成模板。
- **仓库与产物**：仓库目录 `bilibili缓冲插件` → `影哨`；npm 包名 → `yingsao`，发布产物 → `dist/yingsao-<version>.zip`（`verify-release.mjs`、`release-checks.mjs` 的目录层级校验与 store-compliance 坏样本同步更新）。父级 AGENTS.md 的项目路径与 sync-agents 目标同步。
- **刻意不变**（保数据/保契约）：IndexedDB 库名 `bili-buffer-cache` 与镜像库 `bili-buffer-source-ranges-v1`（改名即丢片库）、消息通道 `bili-buffer-playback-assist-v1`、storage 键、alarm 名、页面注入的 `--bili-buffer-*` CSS 变量与 `.bili-buffer-*` 类名、manifest `key`（扩展 ID 与数据归属）。GitHub 远端 slug `Kihara-Ri/bilibili-buffer-extension` 未改，改名后旧链接自动重定向；如需彻底换 slug，再同步 store/README 与隐私页里的链接。
- 未新增权限；版本 2.8.10 → 2.8.11。
- 验证：npm test、npm run check、npm run test:popup、npm run build（产物 `yingsao-2.8.11.zip`）。

## 2.8.10 · 播放页去冗余与系统自决网络策略

- **砍掉「CDN 路线」「最大并发」下拉**：线路固定原清单优先（original）、全局上限固定 32 路，实际并发由 2.8.1 的收益自适应策略在限内自行决定。`sanitizeAssistConfig` 与播放桥的 `normalizedConfig` 都把这两个字段钉死——历史存储里的用户选择（旧 mainland/auto 模式、自定义上限）随下次读取即失效，不会再被页面侧继承；桥原先在字段缺失时回退 `mainland` 的行为一并纠正为 `original`，与文档默认一致。字段仍保留在配置形状里，供预算服务（`getAssistConfig().maxConcurrency`）与统计消费。
- **「进度条外观」展开区去冗余**：摘要行的示意条成为唯一预览；展开后移除重复的「进度条外观」标题与第二条大进度条（连同「配色示意」说明），只保留图例、已播放颜色、单独标出提前加载、预设/自定义配色与说明；「恢复默认」移至底部右对齐。`--played-color` / `--preheat-color` 变量与 `data-highlight="false"` 灰色规则继续同时覆盖摘要条与图例。
- **实现范围**：popup.html/css 重排 + popup.js 移除两个下拉的元素、监听与值同步；`src/assist-config.js`、`src/playback-bridge.js` 钉死字段；`createNetwork` 的选项 API 不变（内部引擎与测试仍可显式传参）。
- **测试契约**：popup-layout 断言两个下拉不存在、无重复标题/第二条进度条；assist-config 与 playback-concurrency-policy 改为断言保存值被钉死覆盖为 original/32。
- 未新增权限；版本 2.8.9 → 2.8.10。
- 验证：npm test（228 例）、npm run check、npm run test:popup、npm run build。

## 2.8.9 · 播放面板紧凑化与下载启动竞态修复

- **问题**：「播放」页把九项指标、说明段落与进度条外观编辑器全部平铺，弹窗要滚很久；多数计数长时间是「0 次」，而最常用的外观配色却沉在最底部，不符合“先看效果、再调参数”的使用习惯。
- **重排**：概览条常驻「加速网速 / 已命中缓存 / 预热就绪」三个回答“有没有效果”的数字；CDN 路线与最大并发压成一行内联选择（低频调节项，语义放悬停提示）；「进度条外观」改为折叠区，摘要行内嵌实时配色预览条——一眼看到当前颜色、点开才是完整编辑器；其余六项诊断计数（播放器缓冲、当前并发、已加速请求、备用节点救援、原线路回退、最近完成节点）收进「详细统计」折叠区。默认态整页一屏放下，全部展开约 1000px。
- **文案**：CDN 路线选项由「大陆优先 / 原线路优先 / 仅原始节点」更名「仅原线路 / 原线路 + 大陆备用 / 大陆优先」（默认项排最前、按介入程度递进），并发标签由「下载并发 / 当前上限」改为「当前并发 / 上限」；两个下拉的悬停提示解释原线路/大陆备用语义与并发的作用边界。
- **实现**：仅 `popup.html` / `popup.css` 重排，`src/popup.js` 无需改动——九个指标节点 ID 全部保留，渲染循环与 CSS 变量注入对 `<details>` 结构同样生效；外观摘要预览与编辑器预览共用 `#assist-appearance` 上的 `--played-color` / `--preheat-color` 变量，`data-highlight="false"` 规则同时覆盖两处。清理了上一版遗留的 `.assist-settings` 死样式。
- **稳定性契约不变**：高频刷新仍只改 `dd` 文本，节点身份、键盘焦点、pending 状态、等宽数码与单行约束全部保留；`popup-layout.test.mjs` 的结构断言改为锁定“概览三项常驻 + 外观/统计折叠”的新契约，`popup-instant-integration.mjs` 与 `run-popup-stability.mjs` 在交互前展开折叠区、并断言默认折叠态。
- **竞态修复（恶性）**：`startDownload` 原先在 `await getVideo`（IndexedDB 读窗口）之后才注册 `activeDownloads`，两条并发 START_DOWNLOAD（连点「开始缓存」，或看门狗恢复与用户启动竞态）会各建一个下载器并互相覆盖注册；先结束的一方还会无条件按键删除另一方的注册，产生不受删除/中止控制的孤儿下载器，可致同任务双份下载、已删除数据被重建。修复为**同步占位再异步校验**，注销改为按 job 身份校验（`releaseDownloadJob`）；`tests/offscreen-integration.mjs` 新增并发 START_DOWNLOAD 恰好启动一个下载器的复现断言。
- **错误分类修复**：`共享下载预算不可用 / 请求预算通道繁忙 / 下载租约已过期 / 下载租约到期` 等预算与租约的暂时性协调错误原先不匹配 `isRecoverableDownloadError`，离线任务会直接进入硬错误而非按退避自动续传；现按可恢复处理（与 2.8.2 的设计意图一致），`tests/download-retry.test.mjs` 补七条消息的分类断言。
- 另：运行时链路审查确认 SW 无跨生命周期定时器依赖、预算租约无泄漏/饿死路径、IndexedDB 事务与进度水位一致、预取调度无永久停死、消息通道 `return true` 语义完整。
- 未新增权限，未改默认配置与网络策略；版本 2.8.8 → 2.8.9。
- 验证：npm test（228 例）、npm run check、npm run test:popup、npm run build。

## 2.8.8 · Chrome 版本兼容门禁、同版验收与运行期自检

- **故障结论**：Chrome 升到 154 后插件不可用。复核确认扩展代码与 154 兼容（用与用户同构建的 Chrome for Testing 154.0.8037.57 跑四个浏览器套件全绿，并用真实扩展 + 本地假 CDN 验证 Offscreen 离线下载 11,534,546 字节、预算租约与 DNR 规则集）；真正原因是该配置里扩展被停用（`disable_reasons = [1]`）且属解包安装，停用后内容脚本、DNR 与 SW 都不运行；而验收只跑在低一档的 153 上，版本漂移无人发现。详见 [浏览器版本兼容与自检](docs/browser-compat.md)。
- **验收夹具修正**：`tests/offscreen-integration.mjs` 与 `tests/offscreen-merge-integration.mjs` 的消息桩自 2.8.2 起只回 `{ok:true}`，下载器拿不到 `lease`，在「等待完成」处静默超时——弹窗与离线下载链路实际没有覆盖。新增 `tests/fixtures/fake-budget.mjs` 直接复用 `src/request-budget.js` 的 `transitionBudget`，并断言租约的申请与归还，后台改策略时夹具不再漂移。
- **Chrome 版本门禁**：`browser-compat.json` 记录已验收主版本、构建号、日期与证据；`npm run check` 增加 `scripts/check-browser-compat.mjs`，本机 Chrome 或验收浏览器跑到未验收主版本时失败并给出下一步命令（探测不到浏览器时明确跳过）。`manifest.minimum_chrome_version` 保持 116：该字段是 API 下限，版本漂移属于上限问题。
- **同版验收**：`npm run test:chrome` 取与本机 Chrome 同版本的 Chrome for Testing（缓存于 `.tmp/cft-cache/`）重跑全部浏览器套件，套件通过 `PLAYWRIGHT_CHROMIUM_EXECUTABLE` 覆盖启动浏览器。
- **运行期自检**：`src/health-check.js` 逐项探测 `storage.local`、`storage.session`、IndexedDB（真实开库）、DNR 静态规则集、`offscreen` + `runtime.getContexts`、`downloads`、`cookies` 与浏览器版本；单项 3 秒超时、任何失败都不向上抛，结论存 `browserHealthV1`（6 小时复用，`chrome_update` 强制刷新）。弹窗只在失败或读不到结论时显示区块并给出 `chrome://extensions` 排查入口；自检无法发现「扩展被停用」，该情况由弹窗固定文案与 README 说明。
- 未新增权限，未改变默认配置与播放路径。
- 验证：npm run check（含版本门禁）、npm test（227 例，含 `tests/health-check.test.mjs`）、npm run test:chrome（153 与 154 各四个浏览器套件全绿）、npm run build。

## 2.8.7 · 兼容与回退

- 缺失 `indexedDB`、`atob`/`btoa`、`window.postMessage`、`crypto.randomUUID` 或 `MutationObserver` 时，相关能力整体停用并退回原生播放路径；镜像服务读写只返回未命中、绝不向上抛错。镜像写入器抽为 `src/source-mirror.js`：按任务配额串行写入，同一任务连续 3 次失败即停写。复用读取超时 1.5 秒、迟到回包丢弃、连续 3 次失败暂停 60 秒。重复 `install` 与重复镜像写入幂等；桥丢弃非法请求且不透传额外字段；默认配置与权限保持不变。详见 docs/compat-fallback.md。
- 验证：`tests/playback-compat.test.mjs`、npm test、npm run check、npm run build。

## 2.8.6 · 持久化源范围复用

- 离线下载中逐个校验过的源范围（`onVerifiedRange`）镜像到 Service Worker 的独立 IndexedDB 库，按「路径 + 总长」建立内容身份，主机与查询串不参与匹配；签名有效期、账户分区（SESSDATA 摘要）与 LRU 上限共同约束，合并/抽音轨产物永不进入。页面侧只对本次会话 playurl 授权过的地址发起读取，要求连续覆盖、单次 ≤ 4 MiB、每页单在途；未命中、超时、连续失败或模块缺失都回到原生网络路径，复用命中单独统计，不污染 CDN 吞吐样本。详见 docs/persistent-range-reuse.md。
- 验证：`tests/source-range-store.test.mjs`、`tests/playback-reuse.test.mjs`、`node tests/run-existing-reuse-probe.mjs`（用户现有 Chrome 真实 IndexedDB）、npm test、npm run check、npm run build。

## 2.8.5 · 字节级内存准入与落盘背压

- 在线工作空间按字节预留、同实例上限 64 MiB，部分拼接总预留 32 MiB，取消与导航释放等待者。驻留块先回收再复制。离线每轨最多 16 MiB 在途/乱序/待提交窗口，事务成功才释放；慢首块和慢磁盘产生背压，首次错误取消同轨请求并唤醒等待者，范围描述惰性生成。边界是插件工作缓冲而非 Chrome RSS，原生回退、已交付播放器的数据及 GC/Blob 内部副本另计，详见 docs/download-memory.md。
...
### 问题十六：Chrome 更新后整个插件「没反应」

**现象**：用户 Chrome 升到 154 后，弹窗打不开、网页侧没有加速、图标点不动，只能得到「貌似不能用了」。

**第一层排查（排除代码不兼容）**：用与用户完全同构建的 Chrome for Testing 154.0.8037.57 加载扩展：MAIN/ISOLATED 注入、播放加速、页面缓存命中、Offscreen 离线下载与 IndexedDB 落盘、共享下载预算租约、DNR 规则集全部正常；仓库四个浏览器套件在 154 上全绿。因此不是 Chrome 154 的 API 变更导致。

**第二层排查（真正的直接原因）**：读用户配置发现扩展处于被停用状态——`Profile 1/Secure Preferences` 里 `extensions.settings.ppoagkhgdcfchiodhgadpenibndbnhcj.disable_reasons = [1]`（`DISABLE_USER_ACTION`），安装形态是解包安装（`location = 4`，路径位于 `Profile 1/UnpackedExtensions/bili-buffer-extension-2.8.7_FQlf2e`）。扩展被停用后内容脚本、DNR 规则与 Service Worker 全部不运行，表现就是「完全没反应」。同时验证过「关闭开发者模式不会自动停用解包扩展」（153/154 均不触发），因此没有证据把这次停用归因于 Chrome 的自动行为，但恢复方式一致：`chrome://extensions` 打开开发者模式并重新启用。这一点写进 README，并作为弹窗自检区块的固定提示文案。

**第三层排查（为什么没有提前发现）**：仓库的浏览器验收跑在 Playwright 自带的 Chrome for Testing 153 上，比用户的 154 低一档；而覆盖弹窗与离线下载的 `npm run test:popup` 自 2026-09-22 引入共享下载预算后就一直是失败的。原因是两个 Offscreen 夹具的消息桩只回 `{ok:true}`，下载器申请不到 `lease`，会一直等到 15 秒预算超时，于是套件在「等待下载完成」处超时——而浏览器套件不在 `npm test` 里，红灯长期无人察觉。两件事叠加，等于「验收全绿」与「插件不可用」可以同时成立。

**修复**：夹具改为直接调用生产状态机 `transitionBudget`（`tests/fixtures/fake-budget.mjs`），并断言租约申请与归还；新增 Chrome 版本兼容门禁（`browser-compat.json` + `scripts/check-browser-compat.mjs`，纳入 `npm run check`）；新增同版验收命令 `npm run test:chrome`；新增运行期浏览器自检 `src/health-check.js`，弹窗在能力缺失时列出具体失败项与排查入口。

**验证**：`npm run check`、`npm test`（227 例）、`npm run test:popup` 在 153/154 均通过；`npm run test:chrome` 自动下载 154.0.8037.57 并跑完四个套件；真实浏览器确认自检全绿且健康时弹窗不显示该区块，模拟能力缺失时区块出现、列出失败项、重复检测不重建节点、恢复后收起。
