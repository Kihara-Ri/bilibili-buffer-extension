...
项目采用 Chrome Manifest V3，按职责拆成四个运行环境：

| 运行环境 | 主要文件 | 职责 |
| --- | --- | --- |
| Popup | `popup.html`、`src/popup.js` | 识别当前页面、显示画质菜单、圆角矩形按钮、进度、速度、错误和本地片库；只负责交互，不承担长时间下载 |
...
| Content Script | `src/content.js` | 使用页面登录会话请求播放清单；发现已完成缓存后，以 Blob 或 MediaSource 交给原页面 `<video>` |
...
画质面板不能只相信 `accept_quality`。该数组可能声明 1080P60、1080P、720P，但匿名响应实际只下发 480P、360P 的 `dash.video`。v1.3.0 改为遍历真实 `dash.video` 表示，只保留具有 URL、MP4 MIME 和编码信息的画质，再按画质代码降序排列，第一项才是“最高可缓存画质”。选定画质后，从同一画质的表示中优先选择兼容性最好的 AVC，其次 AV1、HEVC，并选择可由 MediaSource 播放的最高带宽音频轨。接口如果仍提供单段 MP4，则保留旧路线作为兼容回退。

### 3. IndexedDB 数据结构

缓存数据库名称为 `bili-buffer-cache`，当前 schema 版本为 1，包含两个对象仓库：

- `videos`：旧缓存继续使用 `${bvid}:${cid}`；新缓存使用 `${bvid}:${cid}:q${quality}`，不同画质可以共存；
...
| `downloadMetrics` | v1.6.0 起记录并发数、分块大小、CDN 主机名、请求/重试数、网络字节和 IndexedDB 写入耗时；不额外记录签名 URL 或 Cookie |
...
3. 从主 URL 和最多两个备用 URL 各取 64 KiB 固定 Range 并行探测，仅接受起点、终点和总大小合法的 HTTP 206，按实测字节/耗时排序；单个探测 8 秒超时，不让卡死节点拖住任务；
...
6. 某个分块失败或 30 秒无法完成时，保持相同起止范围切换到下一个 CDN；接收了但失败的字节会从界面临时进度回滚；
...
热更新服务仅用于本地开发。发布 Chrome Web Store 版本前应移除 `src/dev-reload.js`、本机端点权限和对应脚本，但不能更换扩展身份或清除 IndexedDB，否则用户已有缓存无法沿用。
...
- 可选择当前页面登录状态实际返回的 MP4 / DASH 画质，默认选择最高一档；
- 支持 Chrome MediaSource 能解码的 DASH MP4 视频轨和音频轨；
- 默认使用最多 3 路、2 MiB 固定 Range 并发，会先测速候选 CDN，分块失败或超时时自动换节点；
...
单元测试当前共 27 项，在原有 URL、画质、保存、本地播放、固定 Range、CDN 回退、原子续传和原生 `fetch` 上下文覆盖之外，新增 Popup 快照页面键、标签页/视频匹配、30 分钟新鲜度与用户画质保留。浏览器集成测试额外覆盖重复打开零 B 站请求、Cookie 失效、切换视频拒绝旧快照、刷新期间的选择竞态和片库不创建 Offscreen。
